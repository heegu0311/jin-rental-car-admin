...
export default function EventsPage() {
  ...
  const toggleActive = async (id: string, currentStatus: boolean) => {
    ...
    if (error) {
      ...
      refreshPublicSiteOrNotify("노출 상태를 변경했습니다.");
      setEvents(
        events.map((e) =>
  ...
  const deleteEvent = async (id: string) => {
    ...
    if (error) {
      ...
      refreshPublicSiteOrNotify("이벤트를 삭제했습니다.");
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredEvents.map((event) => (
            <div
              key={event.id}
              className="group bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden hover:shadow-lg hover:border-blue-200 transition-all duration-300 flex flex-col"
            >
