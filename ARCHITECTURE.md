...
`site_content`의 공개된 구조화 콘텐츠는 페이지 전체 본문을 대체한다. about/info/accident/privacy/terms 초안 상태에서는 기존 상세 레이아웃을 유지한다. 게시 전에 편집 화면에서 실제 운영 문구를 확인한다. 사업자 연락처·카카오 채널은 `/settings`에서 입력한다. 연락처가 비어 있을 때는 임의 번호를 출력하지 않는다.
