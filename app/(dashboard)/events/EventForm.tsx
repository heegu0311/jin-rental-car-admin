...
const RichEditor = dynamic(
  () => import("@/components/admin/RichEditor").then((mod) => mod.RichEditor),
  {
    ssr: false,
    loading: () => (
      <div className="h-[400px] w-full bg-slate-50 animate-pulse rounded-xl border border-slate-200" />
    ),
  },
);
...
export function EventForm({ initialData }: EventFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const eventData = {
      ...
      start_date: startDate
        ...
        : null,
      end_date: endDate
        ? new Date(`${endDate}T23:59:59+09:00`).toISOString()
        : null,
      is_active: isActive,
      is_popup: isPopup,
    };
  ...
  return (
    ...
    >
      <label className="flex items-center gap-3 rounded-lg border bg-white p-4 text-sm">
        <input
          type="checkbox"
          checked={isPopup}
